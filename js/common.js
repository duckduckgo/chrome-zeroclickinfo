...
DuckDuckBox.prototype = {
  ...
  makeCategoryItem: function(text) {
    var category_item = $('<div>', {
      class: 'ddg_zeroclick_category_item'
    });
  ...
  displayAnswer: function(answer) {
    ...
    if (this.resultsLoaded()) {
      ...
      if (category) {
        ...
        category = category.replace(/(\s.|^.)/g, function(_, word) {
      ...
      var result = $("<div>", {
        id: 'ddg_zeroclick_answer'
      });

      result.append(
        $("<div>", {
          id: 'ddg_zeroclick_answer_title'
        }).html(answer));
      result.append(
        $("<div>", {
          id: "ddg_zeroclick_answer_category"
        }).html(category));
  ...
  displaySummary: function(res, query) {
    ...
    var abstract = (res['Abstract'].length > DDG_SHORTABSTRACT_LEN) ? this.truncate(res['Abstract'],
      DDG_SHORTABSTRACT_LEN) : res['Abstract'];
    var abst = $('<div>', {
    ...
    if (res['Abstract'].length > DDG_SHORTABSTRACT_LEN) {
      ...
      official_links.prepend($('<a>', {
          class: 'ddg_zeroclick_summary_show_more'
        })
        .append($('<i>', {
          class: 'ddg_zeroclick_summary_icon'
        }))
        .append($('<span>', {
            class: 'ddg_zeroclick_summary_expand-more'
          })
          .text('Show more')
        )
        .append($('<span>', {
            class: 'ddg_zeroclick_summary_expand-less'
          })
          .text('Show less')
        )
        .click(function() {
    ...
    for (var i = 0; i < res['RelatedTopics'].length; i++) {
      ...
      if (res['RelatedTopics'][i]['Topics']) {
        ...
        for (var j = 0; j < topics.length; j++) {
          ...
          var icon_disambig = $('<div>', {
            class: 'icon_disambig'
          });
          if (topics[j]['Icon']['URL'])
            icon_disambig.append($('<img>', {
              src: topics[j]['Icon']['URL']
            }));
      ...
      if (res['RelatedTopics'][i]['Icon']['URL']) {
        icon_disambig.append($('<img>', {
          src: res['RelatedTopics'][i]['Icon']['URL']
        }));
      }
  ...
  displayCategory: function(res, query) {
    ...
    for (var i = 0; i < res['RelatedTopics'].length; i++) {
      ...
      var icon_category = $('<div>', {
        class: 'icon_category'
      });

      if (res['RelatedTopics'][i]['Icon']['URL']) {
        icon_category.append($('<img>', {
          src: res['RelatedTopics'][i]['Icon']['URL']
        }))
      }
