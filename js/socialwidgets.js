...
function replaceIndividualButton (key) {
    ...
    var buttonsToReplace = document.querySelectorAll(buttonSelectorsString)

    for (var i = 0; i < buttonsToReplace.length; i++) {
        var buttonToReplace = buttonsToReplace[i]
        var button = createReplacementButtonImage(key, buttonToReplace)
...
function createReplacementButtonImage (key, trackerElem) {
    ...
    button.setAttribute("title", "DuckDuckGo has replaced this " + key + " button")
