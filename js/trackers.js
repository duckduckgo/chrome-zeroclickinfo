...
require.scopes.trackers = (function () {
  ...
  var entityList = load.JSONfromExternalFile(settings.getSetting('entityList'))
  var entityMap =  load.JSONfromLocalFile(settings.getSetting('entityMap'))
  ...
  function isTracker (urlToCheck, currLocation, tabId, request) {
    ...
    if (currLocation.match(/duckduckgo\.com/)) return false
    ...
    if (settings.getSetting('trackerBlockingEnabled')) {
      ...
      if (isFirstPartyRequest(currLocation, urlToCheck)) return
      if (social_block) blockSettings.push('Social')
      ...
      if (trackerByParentCompany) {
        ...
        if (trackerByParentCompany.cancel) return
  ...
  function isRelatedEntity (parentCompany, currLocation) {
    ...
    if (parentEntity && parentEntity.properties) {
      ...
      if (host.match(parentEntity.regexProperties)) return true
    }
